...
export const stat = function (entries, l) {
  l = l ?? entries?.length
  const { key, value } = this
  let i = 0, j = 0,
    k = key.init ? key.init() : entries[i++][0],
    v = value.init ? value.init() : entries[j++][1]
  const keyAcc = key.acc, valueAcc = value.acc
 ...
 */
export const Stat = ({ key, value } = {}) => {
  if (typeof key === FUN) key = { acc: key }
  if (typeof value === FUN) value = { acc: value }
  return stat.bind({ key, value })
}
